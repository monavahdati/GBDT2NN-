...
# ----------------------------
# Step 1.5: Train LightGBM
# ----------------------------
import lightgbm as lgb

params = {'objective': 'binary', 'metric': 'binary_logloss', 'learning_rate': 0.05, 'num_leaves': 31, 'verbose': -1}
lgb_train = lgb.Dataset(X_train_small, label=y_train_small)
lgb_model = lgb.train(params, lgb_train, num_boost_round=100)

# Predict leaf indices
train_leaf_indices = lgb_model.predict(X_train_small, pred_leaf=True)
test_leaf_indices = lgb_model.predict(X_test, pred_leaf=True)

n_leaves_per_tree = [np.max(train_leaf_indices[:, i]) + 1 for i in range(train_leaf_indices.shape[1])]
n_trees_total = train_leaf_indices.shape[1]
n_leaves_per_tree = [np.max(train_leaf_indices[:, i]) + 1 for i in range(train_leaf_indices.shape[1])]
n_trees_total = train_leaf_indices.shape[1]
...
# ----------------------------
class LightGBM_Embedding_DNN(nn.Module):
    def __init__(self, n_leaves_per_tree, original_feat_dim, emb_dim=16, hidden_dim=256):
        super().__init__()
        self.embeddings = nn.ModuleList([
            nn.Embedding(num_leaves, emb_dim) for num_leaves in n_leaves_per_tree
        ])
        self.fc = nn.Sequential(
            nn.Linear(len(n_leaves_per_tree) * emb_dim + original_feat_dim, hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, leaves, original_feats):
        embeds = [self.embeddings[i](leaves[:, i]) for i in range(leaves.shape[1])]
        x = torch.cat(embeds, dim=1)
        x = torch.cat([x, original_feats], dim=1)
        return self.fc(x)

model = LightGBM_Embedding_DNN(n_leaves_per_tree, original_feat_dim=X_train_small.shape[1]).to(device)
...
train_dataset = torch.utils.data.TensorDataset(
    torch.tensor(train_leaf_indices, dtype=torch.long),
...
for epoch in range(num_epochs):
    ...
    for batch_leaves, batch_feats, batch_y in train_loader:
        batch_leaves, batch_feats, batch_y = batch_leaves.to(device), batch_feats.to(device), batch_y.to(device)

        optimizer.zero_grad()
        outputs = model(batch_leaves, batch_feats)
    ...
    # ----------------------------
    # Validation Evaluation (هر epoch)
    # ----------------------------
    model.eval()
    with torch.no_grad():
        X_test_tensor_leaves = torch.tensor(test_leaf_indices, dtype=torch.long).to(device)
        X_test_tensor_feats = torch.tensor(X_test, dtype=torch.float32).to(device)

        outputs_test = model(X_test_tensor_leaves, X_test_tensor_feats)
...
# ----------------------------
model.eval()
X_test_tensor_leaf = torch.tensor(test_leaf_indices, dtype=torch.long).to(device)
X_test_tensor_feat = torch.tensor(X_test, dtype=torch.float32).to(device)

with torch.no_grad():
    outputs = model(X_test_tensor_leaf, X_test_tensor_feat)
...
data.to_csv('customer_credit_offers_GBDT2NN	.csv', index=False)
files.download('customer_credit_offers_GBDT2NN	.csv')
